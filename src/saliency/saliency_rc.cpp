...
Mat CmSaliencyRC::GetRC(cv::Mat &img3f)
{
    return GetRC(img3f, 0.4, 50, 200, 0.5);
}

Mat CmSaliencyRC::GetRC(cv::Mat &img3f, cv::Mat &regIdx1i, int regNum, double sigmaDist)
{
    ...
    finish = clock();
    cout << "quantization Time: " << double(finish - start) / (CLOCKS_PER_SEC)*1000 << " ms" << endl;
    ...
    finish = clock();
    cout << "RegionContrast Time: " << double(finish - start) / (CLOCKS_PER_SEC)*1000 << " ms" << endl;
    ...
    cout << "smooth Time: " << double(finish - start) / (CLOCKS_PER_SEC)*1000 << " ms" << endl;

    return sal1f;
}

Mat CmSaliencyRC::GetRC(cv::Mat &img3f, double sigmaDist, double segK, int segMinSize, double segSigma)
{
    ...
    finish = clock();
    cout << "SegmentImage Time: " << double(finish - start) / (CLOCKS_PER_SEC)*1000 << " ms" << endl;
    return GetRC(img3f, regIdx1i, regNum, sigmaDist);
}

Mat CmSaliencyRC::GetLC(cv::Mat &img3f)
{
    Mat img;
...
Mat CmSaliencyRC::GetSR(cv::Mat &img3f)
{
    Size sz(64, 64);
...
Mat CmSaliencyRC::GetFT(cv::Mat &img3f)
{
...
Mat CmSaliencyRC::GetHC(cv::Mat &img3f) 
{
    // Quantize colors and
...
void CmSaliencyRC::GetHC(cv::Mat &binColor3f, cv::Mat &colorNums1i, Mat &_colorSal)
{
    Mat weight1f;
...
void CmSaliencyRC::SmoothSaliency(cv::Mat &colorNum1i, Mat &sal1f, float delta, const vector<vector<CostfIdx>> &similar)
{
    if (sal1f.cols < 2)
        return;
...
void CmSaliencyRC::SmoothByHist(cv::Mat &img3f, Mat &sal1f, float delta)
{
...
void CmSaliencyRC::SmoothByRegion(Mat &sal1f, cv::Mat &segIdx1i, int regNum, bool bNormalize)
{
...
void CmSaliencyRC::SmoothByGMMs(cv::Mat &_img3f, Mat &_sal1f, int fGmmNum, int bGmmNum, int wkSize)
{
...
void CmSaliencyRC::BuildRegions(cv::Mat& regIdx1i, vector<Region> &regs, cv::Mat &colorIdx1i, int colorNum)
{
...
void CmSaliencyRC::RegionContrast(const vector<Region> &regs, cv::Mat &color3fv, Mat& regSal1d, double sigmaDist)
{	
...
int CmSaliencyRC::Quantize(cv::Mat& img3f, Mat &idx1i, Mat &_color3f, Mat &_colorNum, double ratio, const int clrNums[3])
{
...
Mat CmSaliencyRC::GetBorderReg(cv::Mat &idx1i, int regNum, double ratio, double thr)
{
    // Variance of x and y
...
/************************************************************************/
void CmSaliencyRC::AbsAngle(cv::Mat& cmplx32FC2, Mat& mag32FC1, Mat& ang32FC1)
{
...
/************************************************************************/
void CmSaliencyRC::GetCmplx(cv::Mat& mag32F, cv::Mat& ang32F, Mat& cmplx32FC2)
{
...
#if 0
void CmSaliencyRC::Get(std::string &imgNameW, std::string &salFileDir)
{
    std::vector<std::string> names; 
    string inDir, ext;
    int imgNum = FileOps::GetNamesNE(imgNameW, names, inDir, ext);
...
Mat CmSaliencyRC::Get(cv::Mat &img3f, GET_SAL_FUNC fun, int _wkSize)
{
    Mat sal;
...
void CmSaliencyRC::Evaluate(std::string gtImgsW, std::string &salDir, std::string &resName)
{
    std::vector<std::string> des(SAL_TYPE_NUM);
...
int CmSaliencyRC::Demo(std::string wkDir)
{
    //if (argc != 2) {
