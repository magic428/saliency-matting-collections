...
#if 0
void CmGMM::Demo(std::string &wkDir)
{
	std::string inDir = wkDir + "In/", outDir = wkDir + "Out/";
	vecS namesNE;
...
//#pragma omp parallel for
	for (int i = 0; i < imgNum; i++){
		...
		cv::Mat w1f =  cv::Mat();
		gmm.BuildGMMs(img, idx1i, w1f);
		gmm.RefineGMMs(img, idx1i, w1f);
...
void CmGMM::View(std::string &title, bool decreaseShow)
{
	cv::Mat_<Vec3f> color3f(1, _K);
...
double CmGMM::ViewFrgBkgProb(const CmGMM &fGMM, const CmGMM &bGMM, std::string &title)
{
...
void CmGMM::GetGMMs(std::string &smplW, std::string &annoExt, CmGMM &fGMM, CmGMM &bGMM)
{
	...
	cv::Mat w1f =  cv::Mat();
	fGMM.BuildGMMs(fore3f, fComp1i, w1f);
	fGMM.RefineGMMs(fore3f, fComp1i, w1f);
	fore3f.release();
	fComp1i.release();
	bGMM.BuildGMMs(bg3f, bComp1i, w1f);
	bGMM.RefineGMMs(bg3f, bComp1i, w1f);
}


// show GMM images
void CmGMM::Show(cv::Mat& components1i, std::string& title)
{
