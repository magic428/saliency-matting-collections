...
private:
	...
	void	(*error_function)(const std::string &);
};
...
public:
	/* Constructor. Arguments are the block size and
	   ...
	   will be called if allocation failed; the message
	   passed to this function is "Not enough memory!" */
	DBlock(int size, void (*err_function)(const std::string &) = NULL) { first = NULL; first_free = NULL; block_size = size; error_function = err_function; }
