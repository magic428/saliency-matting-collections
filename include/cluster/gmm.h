...
public:
    ...
    void BuildGMMs(cv::Mat& sampleDf, Mat& component1i, cv::Mat& w1f); 
    int RefineGMMs(cv::Mat& sampleDf, Mat& components1i, cv::Mat& w1f, bool needReAssign = true); // Iteratively refine GMM

    bool Save(std::string &name) const;
    bool Load(std::string &name);
    double GetSumWeight() const {return _sumW;}

    void GetProbs(cv::Mat sampleDf, vector<Mat> &pci1f) const; // Get Probabilities of each Channel i
    void GetProbsWN(cv::Mat sampleDf, vector<Mat> &pci1f) const; // Get Probabilities of each Channel i, without normalize
...
protected:
    ...
    void AssignEachPixel(cv::Mat& sampleDf, Mat &component1i);
};
...
public:
    ...
    void View(std::string &title, bool decreaseShow = true);

    // Show foreground probabilities represented by the GMMs
    static double ViewFrgBkgProb(const CmGMM &fGMM, const CmGMM &bGMM, std::string &title);

    static void GetGMMs(std::string &smplW, std::string &annoExt, CmGMM &fGMM, CmGMM &bGMM);

    static void Demo(std::string &wkDir) {} ;

    // Show GMM images
    void Show(cv::Mat& components1i, std::string& title);
...
template <int D> void CmGMM_<D>::BuildGMMs(cv::Mat& sampleDf, Mat& component1i, cv::Mat& w1f)
{
...
template <int D> int CmGMM_<D>::RefineGMMs(cv::Mat& sampleDf, Mat& components1i, cv::Mat& w1f, bool needReAssign)
{
    bool weighted = w1f.data != NULL;
    int rows = sampleDf.rows, cols = sampleDf.cols; { 
        CV_Assert(sampleDf.data != NULL && sampleDf.type() == CV_MAKETYPE(CV_32F,D));
...
template <int D> void CmGMM_<D>::AssignEachPixel(cv::Mat& sampleDf, Mat &component1i)
{
    int rows = sampleDf.rows, cols = sampleDf.cols;
...
template <int D> cv::Vec<float, D> CmGMM_<D>::getMean(int k) const {
    CV_Assert(k >= 0 && k <= _K);
    Vec3f meanColor;
...
template <int D> bool CmGMM_<D>::Save(std::string &name) const
{
...
template <int D> bool CmGMM_<D>::Load(std::string &name)
{
    ...
    if (strncmp(buf, "CmGMM", 5) != 0)	{
        std::cout << "Invalidate CmGMM data file" << name << std::endl;
        return false;
    }
    int oldK = _K;
...
template <int D> void CmGMM_<D>::GetProbs(cv::Mat sampleDf, vector<Mat> &pci) const
{
    pci.resize(_K);
...
template <int D> void CmGMM_<D>::GetProbsWN(cv::Mat sampleDf, vector<Mat> &pci) const
{
    pci.resize(_K);
    for (int c = 0; c < _K; c++) {// for each component c
        pci[c].create(sampleDf.size(), CV_32F);
...
template <int D> void CmGMM_<D>::iluProbs(cv::Mat sampleDf, std::string &nameNE) const
{
    vector<Mat> pci;
    ...
    for (int i = 0; i < _K; i++){
        ...
        cv::imshow(nameNE + format("%d.png", i), tmpShow);
    }
    // for (int i = _K; i < _MaxK; i++)
        // CmFile::WriteNullFile(nameNE + format("%d.nul", i));
}

template <int D> void CmGMM_<D>::iluProbsWN(cv::Mat sampleDf, std::string &nameNE) const
{
    vector<Mat> pci;
